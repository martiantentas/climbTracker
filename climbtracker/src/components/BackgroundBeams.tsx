...
const BackgroundBeams = React.memo(({ className }: { className?: string }) => {
  return (
    <div className={`pointer-events-none absolute inset-0 h-full w-full${className ? ` ${className}` : ''}`}>
      <svg
        aria-hidden="true"
      ...
      >
        ...
        <defs>
          {pathData.map((_, i) => (
            <linearGradient key={`bb-gradient-${i}`} id={`bb-gradient-${i}`} x1="0%" y1="0%" x2="100%" y2="100%">
